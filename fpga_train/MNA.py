...
class MNA:
    ...
    def export(self,A, xo ,src, M = 32,S = 2):
        K = M
        M_num = 4 * S + 2
...
#        print(R_size)
        ...
        if len(self.switch_list) > 0:
            for s in range(S):
                offset = 2 * K + s * 2 * K
                A[(offset):(offset+R_size),0:R_size] = list(self.switch_list.values())[s].Hoff
                A[(offset + K):(offset + K + R_size),0:R_size] = list(self.switch_list.values())[s].Hon

        if len(self.diode_list) > 0:
            for s in range(S):
                offset = 2 * K + s * 2 * K + S * K * 2
                A[(offset):(offset+R_size),0:R_size] = list(self.diode_list.values())[s].Hoff
                A[(offset + K):(offset + K +R_size),0:R_size] = list(self.diode_list.values())[s].Hon

            for s in range(S):
                offset = K * M_num + 2 * s
                A[(offset),0:R_size] = list(self.diode_list.values())[s].JOff.T
                A[(offset + 1),0:R_size] = list(self.diode_list.values())[s].JON.T
        offset = K * M_num + 2 * S
        src[:,0:R_size] = self.x0[:].T
        xo[:,0:R_size] = self.static[:].T
